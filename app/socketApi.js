...
MongoClient.connect(
  "mongodb+srv://usuario1:usuario1@cluster0-jo7y6.mongodb.net/test?retryWrites=true&w=majority",
  (err, client) => {
...
socketApi.sendNotification = data => {
  db.collection("mensajes").insertOne(data, (err, result) => {
    ...
    socketApi.showMessages().toArray(function(error, documents) {
      documents.forEach(mensaje =>
        mensajes.push({ author: mensaje.author, text: mensaje.text })
      );
